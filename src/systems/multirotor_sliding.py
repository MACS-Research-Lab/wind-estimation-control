...
class MultirotorTrajEnv(SystemEnv):
    ...
    def reset(self, uav_x=None, modify_wind=False):
        ...
        self.state_range[15:18] = self.state_range[:3]
        self.state_range[18:21] = self.state_range[:3]
        ...
        err_next_wp = np.asarray(uav_x[12:15]) 
        err_next_next_wp = np.asarray(uav_x[15:18]) 
        dist = np.asarray(uav_x[18:21])

        self.x = np.concatenate((err_wp, vel, ori, rat, err_next_wp, err_next_next_wp, dist), dtype=self.dtype) # TODO: what impacts does err_wp have on the rest of the code?
        # self.x = np.concatenate((err_wp, vel, ori, rat, err_next_wp), dtype=self.dtype)
        # print(err_next_wp, err_next_next_wp)
        # Manually set underlying vehicle's state
        self.vehicle.state = self.x
        self._des_unit_vec = (0 - err_wp) / (np.linalg.norm(err_wp) + 1e-6) # TODO: this is no longer correct
    ...
    def step(self, u: np.ndarray, **kwargs):
        # self.x = self.unnormalize_state(self.x)
        ...
        for _ in range(self.steps_u):
            ...
            safe_sliding = True
            if safe_sliding: # if this ends up working, clean it up to not redo the calculation, simply store the previous intersection point
                dist = np.linalg.norm(self.x[12:15] - self.x[:3])
                reached = dist <= self._proximity 
                current_v = self.x[:3] - self.prev_waypt
                ...
                x, r, d, *_, i = super().step(np.concatenate(([self.calculate_safe_sliding_bound(self.x[12:15], intersection_point, distance=self.window_distance),u])))
            else:
                x, r, d, *_, i = super().step(np.concatenate(([self.calculate_sliding_bound(self.x[0:3], self.x[12:15], distance=self.window_distance),u])))
            ...
            self.x[18:21] = intersection_point
            ...
            # done = outoftime or reached # figure out how to turn this on for training only
        ...
        return self.normalize_state(self.state), reward, done, *_, i


    def ctrl_fn(self, x):
        return np.zeros(3, self.dtype)

    def calculate_sliding_bound(self, current_pos, reference_point, distance=5):
        ...
        vector_to_reference = reference_point - current_pos
        ...
        if distance_to_reference <= distance:
            ...
            intermediate_point = current_pos + (distance / distance_to_reference) * vector_to_reference
